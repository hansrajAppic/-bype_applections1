...
class _Rechargehistory_screenState extends State<Balancetransferreport_screen> {
    ...
    try {
      ...
      if (Mobile != null) {
        ...
        final historyreport = await MyGlobalFunctions.PostCall(
          '{"MethodName":"balancetransferreport","UserID":"$Mobile ","Password":"$Password", "ToDate":"${toDateController.text}","Size":"${num}","Page":"${currentPage}","FromDate":"${fromDateController.text}"}',
          "$deviceID",
        );
...
void main() {
  runApp(MaterialApp(
    home: Balancetransferreport_screen(),
  ));
}
